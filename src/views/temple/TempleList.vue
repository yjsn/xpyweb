<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input v-model="options.bean.templeNumber" placeholder="寺庙编号" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
    ...
    <el-table
      ...
      style="width: 100%;">
      <el-table-column label="寺庙编号" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.templeNumber }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="寺庙所在省">
        <template slot-scope="scope">
          <span>{{ scope.row.templeProvince }}</span>
        </template>
      </el-table-column>
      <el-table-column label="寺庙所在市">
        <template slot-scope="scope">
          <span>{{ scope.row.templeCity }}</span>
        </template>
      </el-table-column>
      <el-table-column label="寺庙所在区/县">
        <template slot-scope="scope">
          <span>{{ scope.row.templeCounty }}</span>
        </template>
      </el-table-column>
      <el-table-column label="寺庙具体地址">
        <template slot-scope="scope">
          <span>{{ scope.row.templeAddress }}</span>
        </template>
      </el-table-column>
      <el-table-column label="状态">
        <template slot-scope="scope">
          <span>{{ scope.row.status==0?'申请':scope.row.status==1?'启用':scope.row.status==2?'禁用':scope.row.status==3?'申请未通过':'异常' }}</span>
        </template>
      </el-table-column>
